...
def EchoSummer(epoch_i, loss, acc, sat, pred, train=False):
...
def DataSetsEnsurity(batch_size, train_params, test_params):
...
if __name__ == '__main__':
    ...
    DataSetsEnsurity(batch_size, train_params, test_params)
    ...
    with tf.Session() as sess:
        ...
        with open('training/dev=/log.dat'.format(dev=dev),'a') as logfile:
            for epoch_i in np.arange(start_epoch, start_epoch + epochs_n):
                ...
                EchoSummer(epoch_i,train_stats['loss'],train_stats['acc'],train_stats['sat'],train_stats['pred'],train=True)
                ...
                EchoSummer(epoch_i,test_stats['loss'],test_stats['acc'],test_stats['sat'],test_stats['pred'],train=False)

                savepath = 'training/dev={dev}/checkpoints/epoch={epoch}'.format(dev=dev,epoch=int(round(100*np.ceil((epoch_i+1)/100))))
                os.makedirs(savepath, exist_ok=True)
                if save_checkpoints: (sess, savepath)
